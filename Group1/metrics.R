...
CLfraction <- function(dataset, cluster1, cluster2){
  # fraction of clusters in dataset
  freqtable = table(dataset@ident)
  balance = (freqtable[cluster1] + freqtable[cluster2])/sum(freqtable)
  return(balance)
}
...
CalcMetrics <- function(dataset, cluster1, cluster2){
  ...
  metrics = c(NumCells(dataset, cluster1, cluster2),
              ...
              CLfraction(dataset, cluster1, cluster2),
...
runLoom <- function(loomFilePath, clusterpairs){
  ...
  csvFilePath <- gsub("bipolar_", "bipolar_actual_separabilities_", csvFilePath)
  csvFilePath <- gsub("pbmc_", "pbmc_actual_separabilities_", csvFilePath)
